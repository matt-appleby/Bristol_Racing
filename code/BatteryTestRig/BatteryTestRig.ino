...
*************************************************************************
  ...
  GND       to        GND         to        G
  CS        to        13          to        -
  DO        to        D6 9(MISO)    to        -   looks undef
  CLK       to        13(SCK)     to        -
  -         to        2           to        ON

************************************************************************/

// pin A0 14 is v div read
...
void setup() {
  ...
  Serial.println(" ");
  Serial.println("Secounds Amps Volts");

  pinMode(14, INPUT);
}

void loop() {
  float voltage = (analogRead(14)*0.015*1.02);// 5 is multiplied to account for the 2.15k bottom res and 8.13K top res
  Serial.print(millis()/1000);
  Serial.print(" ");
  Serial.print(ISNS20_get_mA()*1.11);//Needs an adjustment factor of 1.11
  Serial.print(" ");
